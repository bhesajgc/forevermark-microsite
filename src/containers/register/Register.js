    ...
    db.collection("users")
      ...
      .then((snap) => {
        if (snap.docs.length > 0) {
          alert("User with same email ID already exists! Please Log In!")
          // dispatch({
          //   type: "DISPLAY ALERT",
          //   item: { display: true, msg: "User with same email ID already exists! Please Log In!" },
          // });
          setTimeout(() => {
            dispatch({
              type: "DISPLAY ALERT",
              item: { display: false, msg: "" },
            });
          }, 4000);
        } else {
          db.collection("users")
            ...
            .then((snap) => {
              if (snap.docs.length > 0) {
                alert("User with same phone number already exists! Please Log In!")
                // dispatch({
                //   type: "DISPLAY ALERT",
                //   item: {
                //     display: true,
                //     msg: "User with same phone number already exists! Please Log In!",
                //   },
                // });
                setTimeout(() => {
                  dispatch({
                ...
                db.collection("users")
                  ...
                  .then((res) => {
                    ...
                    axios
                      .post("https://fm-mail.herokuapp.com/api/sendmail", {
                        email: email,
                        name: firstName,
                      })
                      .then((response) => {
