...
const MeetingDetail = ({ details }) => {
    ...
    useEffect(() => {
        details.user?.get().then((userData) => {
            if (userData.exists) {
                let name = userData.data().firstName + userData.data().lastName;
                setUser(name);
            }
        }).catch(error => alert(error.message));
        details.slot?.get().then((slotData) => {
            if (slotData.exists) {
                let name = (new Date(slotData.data().startTime)).toISOString().substr(0, 19) + " || " + (new Date(slotData.data().endTime)).toISOString().substr(0, 19)
                setSelectedSlot(name);
            }
        }).catch(error => alert(error.message));
        details.vc?.get().then((repData) => {
            if (repData.exists) {
