export const mediaData = new Map();
export const FM_ZoneData = new Map();
export const boothMap = {
  'BoothScreen 1': {
    boothInfo: {
      'XRC_Image_01_TV.001': 'BoothScreen 1', "Play Button_06.014": 'PlayButton', 'XRC_Image_01_TV': 'Chat',
      'Booth Kisok Branding_02': 'URL_Button', 'Booth  Kiosk Branding': 'Broucher',
      'XRC_Image_01_TV1111': 'Sec_Tv', 'XRC_Image_01_Logo': 'logo'
    },
    Transform: {
      name: 'waypoint1',
      posX: 35,
      posY: 1.35,
      posZ: 51,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 2': {
    boothInfo: {
      'XRC_Image_01_TV.003': 'BoothScreen 2', "Play Button_06.013": 'PlayButton', 'XRC_Image_01_TV.041': 'Chat',
      'Booth Kisok Branding_02.002': 'URL_Button', 'Booth  Kiosk Branding.002': 'Broucher',
      'XRC_Image_01_TV1111.003': 'Sec_Tv', 'Asian Star': 'logo'
    },
    Transform: {
      name: 'waypoint2',
      posX: 35,
      posY: 1.35,
      posZ: 38.5,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 3': {
    boothInfo: {
      'XRC_Image_01_TV.002': 'BoothScreen 3', "Play Button_06.012": 'PlayButton', 'XRC_Image_01_TV.040': 'Chat',
      'Booth Kisok Branding_02.001': 'URL_Button', 'Booth  Kiosk Branding.001': 'Broucher',
      'XRC_Image_01_TV1111.002': 'Sec_Tv', 'B N Jewellers': 'logo'
    },
    Transform: {
      name: 'waypoint3',
      posX: 35,
      posY: 1.35,
      posZ: 26,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 4': {
    boothInfo: {
      'XRC_Image_01_TV.004': 'BoothScreen 4', 'Play Button_06.011': 'PlayButton',
      'XRC_Image_01_TV.043': 'Chat', 'Booth Kisok Branding_02.004': 'URL_Button'
      , 'Booth  Kiosk Branding.004': 'Broucher',
      'XRC_Image_01_TV1111.005': 'Sec_Tv', 'B R Designs': 'logo'
    },
    Transform: {
      name: 'waypoint4',
      posX: 35,
      posY: 1.35,
      posZ: 13.5,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 5': {
    boothInfo: {
      'XRC_Image_01_TV.005': 'BoothScreen 5', 'Play Button_06.010': 'PlayButton', 'XRC_Image_01_TV.042': 'Chat',
      'Booth Kisok Branding_02.003': 'URL_Button', 'Booth  Kiosk Branding.003': 'Broucher',
      'XRC_Image_01_TV1111.004': 'Sec_Tv', 'J B Brother': 'logo'
    },
    Transform: {
      name: 'waypoint5',
      posX: 35,
      posY: 1.35,
      posZ: 2,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 6': {
    boothInfo: {
      'XRC_Image_01_TV.006': 'BoothScreen 6', 'Play Button_06.15': 'PlayButton', 'XRC_Image_01_TV.045': 'Chat',
      'Booth Kisok Branding_02.006': 'URL_Button', 'Booth  Kiosk Branding.006': 'Broucher',
      'XRC_Image_01_TV1111.007': 'Sec_Tv', 'K Girdharlal': 'logo'
    },
    Transform: {
      name: 'waypoint6',
      posX: 35,
      posY: 1.35,
      posZ: -11,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 7': {
    boothInfo: {
      'XRC_Image_01_TV.007': 'BoothScreen 7', 'Play Button_06.17': 'PlayButton', 'XRC_Image_01_TV.044': 'Chat',
      'Booth Kisok Branding_02.005': 'URL_Button', 'Booth  Kiosk Branding.005': 'Broucher',
      'XRC_Image_01_TV1111.006': 'Sec_Tv', 'K P Sanghvi': 'logo'
    },
    Transform: {
      name: 'waypoint7',
      posX: 35,
      posY: 1.35,
      posZ: -23,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 8': {
    boothInfo: {
      'XRC_Image_01_TV.008': 'BoothScreen 8', 'Play Button_06.16': 'PlayButton', 'XRC_Image_01_TV.047': 'Chat',
      'Booth Kisok Branding_02.008': 'URL_Button', 'Booth  Kiosk Branding.008': 'Broucher',
      'XRC_Image_01_TV1111.009': 'Sec_Tv', 'KGK Creations': 'logo'
    },
    Transform: {
      name: 'waypoint8',
      posX: 35,
      posY: 1.35,
      posZ: -35,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 9': {
    boothInfo: {
      'XRC_Image_01_TV.009': 'BoothScreen 9', 'Play Button_06.18': 'PlayButton', 'XRC_Image_01_TV.046': 'Chat',
      'Booth Kisok Branding_02.007': 'URL_Button', 'Booth  Kiosk Branding.007': 'Broucher',
      'XRC_Image_01_TV1111.008': 'Sec_Tv', 'Laxmi Diamonds': 'logo'
    },
    Transform: {
      name: 'waypoint9',
      posX: 35,
      posY: 1.35,
      posZ: -47,
      rotX: 1.5708,
      rotY: 1.5708,
      rotZ: 0,
    },
  },
  'BoothScreen 10': {
    boothInfo: {
      'XRC_Image_01_TV.010': 'BoothScreen 10', 'Play Button_06.009': 'PlayButton', 'XRC_Image_01_TV.048': 'Chat',
      'Booth Kisok Branding_02.014': 'URL_Button', 'Booth  Kiosk Branding.014': 'Broucher',
      'XRC_Image_01_TV1111.012': 'Sec_Tv', 'Walking Tree': 'logo'
    },
    Transform: {
      name: 'waypoint10',
      posX: -33,
      posY: 1.35,
      posZ: 51,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 11': {
    boothInfo: {
      'XRC_Image_01_TV.011': 'BoothScreen 11', 'Play Button_06.007': 'PlayButton', 'XRC_Image_01_TV.049': 'Chat',
      'Booth Kisok Branding_02.011': 'URL_Button', 'Booth  Kiosk Branding.011': 'Broucher',
      'XRC_Image_01_TV1111.011': 'Sec_Tv', 'Venus Jewells': 'logo'
    },
    Transform: {
      name: 'waypoint11',
      posX: -33,
      posY: 1.35,
      posZ: 38.5,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 12': {
    boothInfo: {
      'XRC_Image_01_TV.012': 'BoothScreen 12', 'Play Button_06.008': 'PlayButton', 'XRC_Image_01_TV.055': 'Chat',
      'Booth Kisok Branding_02.012': 'URL_Button', 'Booth  Kiosk Branding.012': 'Broucher',
      'XRC_Image_01_TV1111.013': 'Sec_Tv', 'Veekay diamonds': 'logo'
    },
    Transform: {
      name: 'waypoint12',
      posX: -33,
      posY: 1.35,
      posZ: 26.5,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 13': {
    boothInfo: {
      'XRC_Image_01_TV.013': 'BoothScreen 13', 'Play Button_06.006': 'PlayButton', 'XRC_Image_01_TV.056': 'Chat',
      'Booth Kisok Branding_02.015': 'URL_Button', 'Booth  Kiosk Branding.015': 'Broucher',
      'XRC_Image_01_TV1111.014': 'Sec_Tv', 'Thea Creations': 'logo'
    },
    Transform: {
      name: 'waypoint13',
      posX: -33,
      posY: 1.35,
      posZ: 14,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 14': {
    boothInfo: {
      'XRC_Image_01_TV.014': 'BoothScreen 14', 'Play Button_06.001': 'PlayButton', 'XRC_Image_01_TV.052': 'Chat',
      'Booth Kisok Branding_02.013': 'URL_Button', 'Booth  Kiosk Branding.013': 'Broucher',
      'XRC_Image_01_TV1111.017': 'Sec_Tv', 'Sheetal Manufacturing': 'logo'
    },
    Transform: {
      name: 'waypoint14',
      posX: -33,
      posY: 1.35,
      posZ: 2,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 15': {
    boothInfo: {
      'XRC_Image_01_TV.015': 'BoothScreen 15', 'Play Button_06.002': 'PlayButton', 'XRC_Image_01_TV.054': 'Chat',
      'Booth Kisok Branding_02.017': 'URL_Button', 'Booth  Kiosk Branding.017': 'Broucher',
      'XRC_Image_01_TV1111.016': 'Sec_Tv', 'Indigo Jewellers': 'logo'
    },
    Transform: {
      name: 'waypoint15',
      posX: -33,
      posY: 1.35,
      posZ: -11,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 16': {
    boothInfo: {
      'XRC_Image_01_TV.016': 'BoothScreen 16', 'Play Button_06.003': 'PlayButton', 'XRC_Image_01_TV.051': 'Chat',
      'Booth Kisok Branding_02.016': 'URL_Button', 'Booth  Kiosk Branding.016': 'Broucher',
      'XRC_Image_01_TV1111.015': 'Sec_Tv', 'Rosy Blue': 'logo'
    },
    Transform: {
      name: 'waypoint16',
      posX: -33,
      posY: 1.35,
      posZ: -23,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 17': {
    boothInfo: {
      'XRC_Image_01_TV.017': 'BoothScreen 17', 'Play Button_06.004': 'PlayButton', 'XRC_Image_01_TV.050': 'Chat',
      'Booth Kisok Branding_02.010': 'URL_Button', 'Booth  Kiosk Branding.010': 'Broucher',
      'XRC_Image_01_TV1111.010': 'Sec_Tv', 'Prism Enterprises': 'logo'
    },
    Transform: {
      name: 'waypoint17',
      posX: -33,
      posY: 1.35,
      posZ: -35,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
  'BoothScreen 18': {
    boothInfo: {
      'XRC_Image_01_TV.018': 'BoothScreen 18', 'Play Button_06.005': 'PlayButton',
      'XRC_Image_01_TV.053': 'Chat',
      'Booth Kisok Branding_02.009': 'URL_Button',
      'Booth  Kiosk Branding.009': 'Broucher',
      'XRC_Image_01_TV1111.018': 'Sec_Tv', 'Mahendra Brothers': 'logo'
    },
    Transform: {
      name: 'waypoint18',
      posX: -33,
      posY: 1.35,
      posZ: -47.5,
      rotX: 1.5708,
      rotY: 4.712,
      rotZ: 0,
    },
  },
};

export const fmZone = {
  'Entry 1': {
    Transform: {
      name: 'fmWaypoint1',
      posX: 1.6,
      posY: 1.35,
      posZ: -17,
      rotX: 1.5708,
      rotY: 0,
      rotZ: 0,
    },
  },

  'Entry 2': {
    Transform: {
      name: 'fmWaypoint2',
      posX: 1.6,
      posY: 1.35,
      posZ: 17,
      rotX: 1.5708,
      rotY: 3.142,
      rotZ: 0,
    },
  },
};

export const stairsData = {
  'groundStair1': {
    Transform: {
      name: 'gstair1',
      posX: -18,
      posY: 1.35,
      posZ: -72,
      rotX: 1.5708,
      rotY: -0.1745,
      rotZ: 0,
    },
  },
  'groundStair2': {
    Transform: {
      name: 'gstair2',
      posX: 22,
      posY: 1.35,
      posZ: -72,
      rotX: 1.5708,
      rotY: 0.3491,
      rotZ: 0,
    },
  },
  'upStair1': {
    Transform: {
      name: 'upstair1',
      posX: -43.5,
      posY: 17.3,
      posZ: 8,
      rotX: 1.5708,
      rotY: 3.142,
      rotZ: 0,
    },
  },
  'upStair2': {
    Transform: {
      name: 'upstair2',
      posX: 47,
      posY: 17.3,
      posZ: 8,
      rotX: 1.5708,
      rotY: 3.142,
      rotZ: 0,
    },
  },
};
export const FMZone = {
  'Partner Testimonials': {
    'Inner Brandings.001': 'ZoneVideoScreen',
    'Play Button_05': 'ZonePlayButton',
  },
  'What\'s New?': {
    'XRC_Image_01_TV.024': 'Watering Hole',
    'Play Button_11.001': 'COO(Video)',
    'XRC_Image_01_TV.023': 'COO(PDF)',
    'XRC_Image_01_TV.026': 'Jewellery Certificate',
    'XRC_Image_01_TV.029': 'New Logo',
    'Inner Brandings.008': 'AI Interactive Video',
    'Eye Button_09': 'AI Interactive Video Play',
    'Inner Brandings.009': 'Building Forever Video', //Common Mesh
    'Eye Button_08': 'Building Forever Play',
    'Eye Button_04.004': 'Building Forever PDF',
    'Eye Button_09.001': 'Pursuit of Beauty Play', //Common Mesh
    'Inner Brandings.013': 'Pursuit of Beauty Video', //Common Mesh
  },
  'Yash Innovation': {
    'Eye Button_08.001': 'Yash Technovation Play',
    'Inner Brandings.012': 'Yash Technovation Video', //Common Mesh
  },
  'Brand Corner': {
    'Inner Brandings.006': 'Brand Corner Video Entry 1',
    'Play Button _04': 'Brand Corner Play Entry 1',
    'Inner Brandings.011': 'Brand Corner Video Entry 2',
    'Play button_12': 'Brand Corner Play Entry 2',
  },
  'Consumer Marketing': {
    'Inner Brandings.005': 'Consumer Marketing Video',
    'Play Button_03': 'Consumer Marketing Play'
  },
  'Virtual Showrooms': {
    'XRC_Image_01_TV.035': 'Forever Boutique',
    'XRC_Image_01_TV.031': 'Obses Virtual Store'
  },
  'Designs & Collections': {
    'XRC_Image_01_TV.025': 'Forever Mark Big Ideas url',
    'Inner Brandings.007': 'Trender and Red Carpet Video',
    'Play Button_02': "Trender and Red Carpet Play",
    'Eye Buton.002': 'Trender and Red Carpet PDF 1',
    'Eye Buton.001': 'Trender and Red Carpet PDF 2'
  },
  'Forevermark Training': {
    'XRC_Image_01_TV.039': 'PDF',
    'Inner Brandings.010': 'Video',
    'Play Button_06': 'Play Button',
    'XRC_Image_01_TV.034': 'URL'
  },
  'New Campaigns': {
    'XRC_Image_01_TV.036': 'COT PDF',
    'Play Button_07.001': 'COT Play',
    'XRC_Image_01_TV.032': 'TS PDF',
    'Play Button_07.002': 'TS Play',
    'XRC_Image_01_TV.027': 'Icon Fashion PDF',
    'Play Button_07.003': 'Icon Fashion Play',
    'XRC_Image_01_TV.028': "URL"
  },
  'Consumer Insights': {
    'XRC_Image_01_TV.033': 'Consumer Insights PDF'
  },
  'Gamification': {
    'Inner Brandings.002': 'Screen',
    'Inner Brandings.003': 'Screen',
    'Inner Brandings.004': 'Screen',
    'Game Button': 'Game Button',
    'Game Button.002': 'Game Button',
    'Game Button.001': 'Game Button'
  },
  'Grand Launch': {
    'Eye Buton.003': 'PDF',
    'Blue_02.001': 'URL',
    'Play Button_02.001': 'Play',
    'Eye Buton.005': 'PDF',
    'Blue_02.002': 'URL',
    'Play Button_02.003': 'Play'
  },
  'Visual Search Tool': {
    'XRC_Image_01_TV.030': 'URL',
    'Play Button_07': 'Play',
    'Eye Button_04': 'PDF'
  }
}

export const advertisementData = {
  time: '16:11',
  URL: 'https://www.forevermark.com/en-us/',
};
